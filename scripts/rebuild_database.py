...
def rebuild_database(req):
    '''Rebuild the dataset'''
    dataset = rospy.get_param('~dataset', '/home/ros/catkin_ws/src/avatar/faces')
    encodingFile = rospy.get_param('~encodings', '/home/ros/catkin_ws/src/avatar/faces.pickle')
    method = rospy.get_param('~method', 'hog')
    info = rospy.get_param('~info', '/home/ros/catkin_ws/src/avatar/info.json')
    rospy.loginfo(f"avatar: rebuild_database_server called with dataset {dataset} {info} {encodingFile} method {method}")
...
def warn():
    rospy.loginfo(f"avatar: rebuild_database_server shutting down (killed)")
...
def rebuild_database_server():
    ...
    rospy.loginfo(f"avatar: rebuild_database_server is running")
